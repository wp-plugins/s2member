...
*/
if (version_compare (PHP_VERSION, "5.2", ">=") && version_compare (get_bloginfo ("version"), "2.8.4", ">=") && !isset ($GLOBALS["WS_PLUGIN__"]["s2member"]) && function_exists ("curl_init") && !function_exists ("wp_new_user_notification"))
	{
		/*
...
*/
else if (is_admin () || ($_GET["preview"] && $_GET["template"]))
	{
		...
		else if (function_exists ("wp_new_user_notification"))
			{
				register_shutdown_function (create_function ('', 'echo \'<script type="text/javascript">alert(\\\'Another plugin is conflicting with s2Member. s2Member needs pluggable function: wp_new_user_notification().\\\');</script>\';'));
			}
	}
?>