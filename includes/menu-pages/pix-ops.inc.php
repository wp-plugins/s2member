...
echo '<h2><div>Developed by <a href="' . ws_plugin__s2member_parse_readme_value ("Plugin URI") . '"><img src="' . $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["dir_url"] . '/images/brand-light.png" alt="." /></a></div>s2Member Pixel Tracking Options</h2>' . "\n";
/**/
...
echo '<p>If you use affiliate software, have tracking codes from advertising networks or the like, you\'ll want to read this section. The HTML code that you enter below will be loaded up in the browser when the user lands on the Signup Confirmation Page after checkout. The s2Member Signup Confirmation Page ( also known as a Thank-You Page ), is really just a blank page that alerts the user via JavaScript. It says: `Thank You! Your membership has been approved.`. Then it says: `The next step is to Register a Username. Please click OK to be redirected.`. After they\'ve clicked OK on this alert, and after everything ( if there is anything ) on the blank page has finished loading, they\'ll be redirected to the Registration system, where they will be able to setup their Username and login for the first time. This process has been fully automated, very simple.</p>' . "\n";
echo '<p>Now, if you want to track the performance of your marketing efforts using Google® Analytics, affiliate software or advertising networks, you can place the code for those in the field below. All of the code in the field below will be injected into the Signup Confirmation Page, between the &lt;body&gt;&lt;/body&gt; tags. After everything in your code has finished loading, the new Member will be redirected to the Registration system so they can setup their Username and login. In other words, the Signup Confirmation Page makes sure that any 1x1 pixel images or other data from your Pixel Tracking Code ( in the field below ) has finished loading before it redirects the Member away from the page. This way you can be sure all of your tracking statistics and other efforts remain accurate. This is handled through the magic of the window.onload event, compatible in all major browsers.</p>' . "\n";
/**/
...
echo '</div>' . "\n";
/**/
echo '<div class="ws-menu-page-hr ws-plugin--s2member-other-methods-section-hr"></div>' . "\n";
/**/
...
echo '<p>You may also want to check the s2Member API Options panel. You\'ll find additional layers of automation available through the use of the `Signup`, `Payment`, `EOT/Deletion` and `Refund/Reversal` notifications that are available to you through the s2Member API. These make it easy to integrate with 3rd party applications like affiliate programs and other back-office routines. Because the s2Member API operates silently on the back-end, in conjunction with the PayPal® IPN system, they tend to be more reliable and also more versatile. That being said, nothing really replaces the simplicity of using Pixel Tracking, and the s2Member API/Urls are not necessarily the best tool for the job in all cases. The API/Urls will not work with Google® Analytics, or 1 pixel &lt;img&gt; tags. They operate silently behind-the-scene using cURL connections, as opposed to being loaded in a browser.</p>' . "\n";
echo '</div>' . "\n";
/**/
echo '<div class="ws-menu-page-hr"></div>' . "\n";
/**/
