...
= If you currently run s2Member v2.5 or lower. =
* ( This only applies if you've been using s2Member v2.5 or lower, to protect downloadable files. ) If you're upgrading from a previous version of s2Member, prior to v2.6, please take note that starting with s2Member v2.6, all of your security-enabled files should be placed into a directory outside the main plugin folder before you upgrade. The new directory is located inside `wp-content/plugins/s2member-files/`.
