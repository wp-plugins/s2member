...
*/
$GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["menu-r"] = array ("tools" => true, "tips" => false, "donations" => true);
/*
		...
		foreach ($GLOBALS["WS_PLUGIN__"]["s2member"]["o"] as $key => &$value)
			{
				...
				/**/
					...
					$value = $default_options[$key];
				/**/
				else if ($key === "paypal_business" && !is_string ($value))
					$value = $default_options[$key];
				/**/
				else if ($key === "paypal_identity_token" && !is_string ($value))
					$value = $default_options[$key];
				/**/
				...
				/**/
					...
					$value = $default_options[$key];
				/**/
				else if ($key === "signup_pixel_tracking_codes" && !is_string ($value))
					$value = $default_options[$key];
				/**/
				else if ($key === "signup_notification_urls" && !is_string ($value))
					$value = $default_options[$key];
				/**/
				else if ($key === "payment_notification_urls" && !is_string ($value))
					$value = $default_options[$key];
				/**/
				else if ($key === "eot_del_notification_urls" && !is_string ($value))
					$value = $default_options[$key];
				/**/
				else if ($key === "ref_rev_notification_urls" && !is_string ($value))
					$value = $default_options[$key];
				/**/
				...
				/**/
					...
					$value = $default_options[$key];
				/**/
				else if (preg_match ("/level[1-4]_ruris$/", $key) && !is_string ($value))
					$value = $default_options[$key];
				/**/
