		...
		ws_plugin__s2member_paypalButtonGenerate = function(level)
			{
				...
				var recurring = (term.match (/-NR$/i)) ? '0' : '1';
				term = term.replace (/-NR$/i, '');
				/**/
				if (parseInt(trialPeriod) <= 0)
					code.val (code.val ().replace (/(\<input type\="hidden" name\="(a1|p1|t1)" value\="(.*?)"\>)$/gm, "<!--$1-->"));
				else /* Otherwise, we need to make sure they get un-commented. */
					code.val (code.val ().replace (/\<\!--(\<input type\="hidden" name\="(a1|p1|t1)" value\="(.*?)"\>)--\>$/gm, "$1"));
				/**/
				code.val (code.val ().replace (/name\="src" value\="(.*?)"/, 'name="src" value="' + recurring + '"'));
