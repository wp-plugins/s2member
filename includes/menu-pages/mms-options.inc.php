		...
		if (apply_filters ("ws_plugin__s2member_during_mms_options_page_during_left_sections_display_mms_registration", true, get_defined_vars ()))
			{
				...
				echo '<p>( <em>Highly recommended!</em> ) <strong>Before you go live</strong>, please contact <a href="' . esc_attr (c_ws_plugin__s2member_readmes::parse_readme_value ("Pro Module / Prices")) . '" target="_blank" rel="external">s2Member.com</a> for full documentation. There is some additional functionality that can be enabled for security on a Blog Farm installation; and also some menus/documentation/functionality that can be disabled. NOTE ~ You will be asked to make a <a href="' . esc_attr (c_ws_plugin__s2member_readmes::parse_readme_value ("Donate link")) . '" target="_blank" rel="external">donation</a>, or to purchase our <a href="' . esc_attr (c_ws_plugin__s2member_readmes::parse_readme_value ("Pro Module / Prices")) . '" target="_blank" rel="external">Network Support Package</a>.</p>' . "\n";
				echo '</div>' . "\n";
				echo '</div>' . "\n";
				/**/
				echo '<table class="form-table">' . "\n";
